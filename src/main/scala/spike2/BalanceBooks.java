...
public class BalanceBooks implements Closeable {
    ...
    private static final byte[] TABLE = Bytes.toBytes("mbalances");
            ...
            try (ResultScanner scanner = table.getScanner(new Scan())) {
                for (Result r : scanner) {
                    if (!r.isEmpty()) {
                        ...
                        System.out.println("---------------------Row Id : " + rowId + "Clients : " + totalClients + "Balance : " + balance);
    ...
    public static void main(String[] args) {
        try (BalanceBooks bb = new BalanceBooks(4, 3)) {
            bb.init();
            bb.run();
            bb.verify();
        } catch (Exception e) {
...
//                throw new IOException("Aborting manually...");
                txContext.finish();
