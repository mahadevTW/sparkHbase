...
public class BalanceBooks implements Closeable {
    ...
    private static final byte[] TABLE = Bytes.toBytes("emp");
    private static final byte[] FAMILY = Bytes.toBytes("personal");
    private static final byte[] COL = Bytes.toBytes("name");
    ...
    private BalanceBooks(int totalClients, int iterations, Configuration conf) {
...
//        createTableIfNotExists(conf, TABLE, new byte[][]{FAMILY});
    ...
    public void run() throws IOException, InterruptedException {
        ...
        Client c = new Client(0, totalClients, iterations);
...
//            c.start();
    ...
    public static void main(String[] args) {
        ...
        try (BalanceBooks bb = new BalanceBooks(1, 1, config)) {
            bb.init();
            bb.run();
            bb.verify();
        } catch (Exception e) {
